...
export const metadata: Metadata = {
  title: 'Meet Your Strategy Coaching Team — Frontera',
  description: 'Six world-class AI coaching personas with distinct expertise, personality, and strategic philosophy.',
              ...
              >
                ...
                <div className="relative p-8 md:p-10 flex flex-col md:flex-row gap-8 md:gap-12 items-start">
                  <div className="flex-shrink-0 flex flex-col items-center gap-4">
                    ...
                    >
                      <Image src={coach.avatar} alt={coach.name} width={112} height={112} className="w-full h-full object-cover" />
                      <div className="absolute bottom-1.5 right-1.5 w-3.5 h-3.5 rounded-full border-2 border-[#0d1117]" style={{ background: coach.color.accent }} />
                    </div>
                    <span className="font-mono text-[10px] tracking-[0.2em] text-white/20">0{idx + 1}</span>
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="mb-4">
                      <h3 className="font-display text-3xl md:text-4xl text-white/90">{coach.name}</h3>
                      <span className="font-mono text-xs tracking-wide" style={{ color: coach.color.accent }}>{coach.title}</span>
                    </div>
                    <div className="inline-block px-3 py-1 rounded-full text-xs font-body font-medium mb-5 border" style={{ color: coach.color.accent, borderColor: coach.color.border, background: coach.color.bg }}>
                      {coach.personality}
                    </div>
                    <p className="font-body text-white/45 leading-relaxed mb-6 max-w-2xl">{coach.description}</p>
                    <blockquote className="border-l-2 pl-4 mb-6" style={{ borderColor: coach.color.accent }}>
                      <p className="font-display text-lg italic text-white/60">&ldquo;{coach.tagline}&rdquo;</p>
                    </blockquote>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {coach.bestFor.map((item) => (
                        <span key={item} className="font-mono text-[10px] tracking-wider uppercase px-3 py-1.5 rounded-lg border text-white/30" style={{ borderColor: 'rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)' }}>
                          {item}
                        </span>
                      ))}
                    </div>
              ...
              >
                ...
                <div className="relative p-8 md:p-10 flex flex-col md:flex-row gap-8 md:gap-12 items-start">
                  <div className="flex-shrink-0 flex flex-col items-center gap-4">
                    ...
                    <span className="font-mono text-[10px] tracking-[0.2em] text-white/20">0{idx + 4}</span>
                  </div>
        ...
        <div className="relative max-w-3xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-3 mb-12">
            {[...STRATEGIC_COACHES, ...SPARRING_PARTNERS].map((coach, idx) => (
              ...
              >
                <Image src={coach.avatar} alt={coach.name} width={48} height={48} className="w-full h-full object-cover" />
              </div>
            ))}
          </div>
          ...
          <div className="mt-24 pt-8 border-t border-white/5">
            <div className="flex items-center justify-center gap-3">
              <Image src="/frontera-logo-F.jpg" alt="Frontera" width={28} height={28} className="rounded-lg" />
              <span className="font-body text-sm text-white/25">Frontera Product Strategy Coach</span>
            </div>
          </div>
        </div>
      </section>
