...
// POST /api/product-strategy-agent-v2/phase
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Auto-create strategy version snapshot on phase completion
    // (fire-and-forget — don't block the response)
    if (previousPhase && PHASE_ORDER[phase] > PHASE_ORDER[previousPhase]) {
      try {
        const snapshotUrl = new URL('/api/product-strategy-agent-v2/versions', req.url);
        ...
        // Silently ignore snapshot setup failures
      }
    }
...
// GET /api/product-strategy-agent-v2/phase/quality-gate?conversation_id={id}&phase={phase}
