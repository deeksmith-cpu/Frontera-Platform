...
/**
 * GET /api/product-strategy-agent/activation
 * List all activation artefacts for the current org.
 */
...
/**
 * POST /api/product-strategy-agent/activation
...
export async function POST(req: NextRequest) {
  ...
  switch (type) {
    case 'team_brief': {
      ...
      let thesis = null;
      if ((bet as Record<string, unknown>).strategic_thesis_id) {
        const { data: thesisData } = await supabase
          .from('strategic_theses')
          .select('*')
          .eq('id', (bet as Record<string, unknown>).strategic_thesis_id)
          .single();
        thesis = thesisData;
      }
      ...
      prompt = buildTeamBriefPrompt({
        bet,
        thesis,
        synthesisContext: (synthesis as Record<string, unknown>)?.executive_summary as string | undefined,
        companyName,
      });
      artefactTitle = `Team Brief: ${(bet as Record<string, unknown>).bet?.toString().substring(0, 60) || 'Strategic Bet'}`;
      break;
    }
    case 'guardrails': {
      const tensions = ((synthesis as Record<string, unknown>)?.tensions as Array<{ title?: string; description: string }>) || [];
      prompt = buildGuardrailsPrompt({
        bets: bets.map((b: Record<string, unknown>) => ({ bet: b.bet as string, job_to_be_done: b.job_to_be_done as string })),
        tensions,
        synthesisHighlights: (synthesis as Record<string, unknown>)?.executive_summary as string | undefined,
        companyName,
      });
    ...
    case 'okr_cascade': {
      prompt = buildOKRPrompt({
        bets: bets.map((b: Record<string, unknown>) => ({
          bet: b.bet as string,
          success_metric: b.success_metric as string,
          job_to_be_done: b.job_to_be_done as string,
        })),
        companyName,
      });
    ...
    case 'decision_framework': {
      ...
      prompt = buildDecisionFrameworkPrompt({
        guardrails,
        bets: bets.map((b: Record<string, unknown>) => ({ bet: b.bet as string })),
        companyName,
      });
