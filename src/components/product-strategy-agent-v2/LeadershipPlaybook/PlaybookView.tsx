...
export function PlaybookView({ conversationId }: PlaybookViewProps) {
    ...
    async function fetchPlaybook() {
      try {
        const response = await fetch(
          `/api/product-strategy-agent-v2/leadership-playbook?conversation_id=${conversationId}`
        );
        if (response.ok) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/leadership-playbook', {
        method: 'POST',
