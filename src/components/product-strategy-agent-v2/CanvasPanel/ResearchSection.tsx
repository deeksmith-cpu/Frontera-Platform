...
export function ResearchSection({ conversation, onResearchContextChange }: ResearchSectionProps) {
  ...
  const [microSyntheses, setMicroSyntheses] = useState<Record<string, MicroSynthesisResult>>({});
  const synthesisTriggeredRef = useRef<Set<string>>(new Set());

  // Auto-trigger micro-synthesis when a territory becomes fully mapped
  const triggerMicroSynthesis = useCallback(async (territory: 'company' | 'customer' | 'competitor') => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/micro-synthesis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversation_id: conversation.id, territory }),
      });
      if (response.ok) {
      ...
      console.error(`Micro-synthesis error for ${territory}:`, error);
      synthesisTriggeredRef.current.delete(territory);
    }
  }, [conversation.id]);

  // Check for fully-mapped territories and trigger micro-synthesis
  useEffect(() => {
    const territories: Array<'company' | 'customer' | 'competitor'> = ['company', 'customer', 'competitor'];
    territories.forEach((territory) => {
      const areas = territoryInsights.filter((t) => t.territory === territory && t.status === 'mapped');
      if (areas.length >= 3 && !microSyntheses[territory]) {
        triggerMicroSynthesis(territory);
      }
    });
  }, [territoryInsights, microSyntheses, triggerMicroSynthesis]);

  // Load existing micro-synthesis from framework_state on mount
  useEffect(() => {
    const frameworkState = conversation.framework_state as Record<string, unknown> | null;
    const existing = frameworkState?.microSynthesisResults as Record<string, MicroSynthesisResult> | undefined;
    if (existing) {
      setMicroSyntheses(existing);
      Object.keys(existing).forEach((t) => synthesisTriggeredRef.current.add(t));
    }
          ...
          {(['company', 'customer', 'competitor'] as const).map((territory) => {
            ...
            return (
              <TerritorialInsightSummary
                ...
                territory={territory}
                findings={synthesis}
                onViewDetails={() => handleTerritoryClick(territory)}
              />
            );
          })}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <TerritoryCard
            ...
            mappedAreas={territoryInsights.filter((t) => t.territory === 'company' && t.status === 'mapped').length}
          />
          ...
          <TerritoryCard
            ...
            mappedAreas={territoryInsights.filter((t) => t.territory === 'customer' && t.status === 'mapped').length}
          />
          ...
          <TerritoryCard
            ...
            mappedAreas={territoryInsights.filter((t) => t.territory === 'competitor' && t.status === 'mapped').length}
          />
        </div>
      )}
