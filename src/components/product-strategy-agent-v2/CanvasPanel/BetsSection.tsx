...
export function BetsSection({ conversation }: BetsSectionProps) {
  ...
  const fetchBets = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/product-strategy-agent-v2/bets?conversation_id=${conversation.id}`
      );
      if (response.ok) {
  ...
  useEffect(() => {
    async function fetchSynthesis() {
      try {
        const response = await fetch(
          `/api/product-strategy-agent-v2/synthesis?conversation_id=${conversation.id}`
        );
        if (response.ok) {
  ...
  const handleGenerateBets = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/bets/generate', {
        method: 'POST',
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/bets/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversation_id: conversation.id }),
      });
  ...
  const handleCreateStrategy = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/bets/strategy-document/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleExportStrategy = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/product-strategy-agent-v2/bets/strategy-document/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          conversation_id: conversation.id,
