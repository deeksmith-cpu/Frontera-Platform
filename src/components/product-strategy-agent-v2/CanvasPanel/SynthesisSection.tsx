...
import { StrategyOnAPage } from './StrategyOnAPage';
...
export function SynthesisSection({ conversation }: SynthesisSectionProps) {
  ...
  useEffect(() => {
    async function fetchDebateDecisions() {
      try {
        const response = await fetch(
          `/api/product-strategy-agent-v2/debate-decisions?conversation_id=${conversation.id}`
        );
        if (response.ok) {
          const data = await response.json();
  ...
  const handleDebateDecision = useCallback(async (decision: DebateDecision) => {
    try {
      const response = await fetch('/api/product-strategy-agent-v2/debate-decisions', {
        method: 'POST',
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/product-strategy-agent-v2/synthesis/export?conversation_id=${conversation.id}`
      );
    ...
    <div className="synthesis-section space-y-6">
      ...
      {synthesis && (
        <>
          ...
          {/* Strategy on a Page */}
          {synthesis.opportunities.some((o) => o.ptw) && (
            <StrategyOnAPage
              synthesis={synthesis}
