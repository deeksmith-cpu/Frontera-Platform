...
export function DiscoverySection({ conversation, clientContext, onClientContextUpdate }: DiscoverySectionProps) {
  ...
  const saveContext = async () => {
    ...
    try {
      const res = await fetch('/api/product-strategy-agent-v2/client-context', {
        method: 'PATCH',
  ...
  const handleDeleteMaterial = async (materialId: string) => {
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/product-strategy-agent-v2/materials?id=${materialId}`, {
        method: 'DELETE',
      });
  ...
  useEffect(() => {
    async function fetchMaterials() {
      try {
        const response = await fetch(`/api/product-strategy-agent-v2/materials?conversation_id=${conversation.id}`);
        if (response.ok) {
