      ...
      <ol className="space-y-4">
        {recommendations.map((recommendation, idx) => (
          <li key={idx} className="flex items-start gap-4">
            {/* Number Badge */}
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-600 to-cyan-600 flex items-center justify-center flex-shrink-0 shadow-md">
            ...
            <div className="flex-1 pt-1">
              ...
              {/* Priority Indicator */}
              {idx === 0 && (
                <span className="inline-block mt-2 text-xs font-semibold text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded-full">
                  Highest Priority
                </span>
              )}
            </div>
