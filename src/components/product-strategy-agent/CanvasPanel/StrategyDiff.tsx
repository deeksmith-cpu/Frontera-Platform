...
export function StrategyDiff({ conversationId }: StrategyDiffProps) {
  ...
  async function loadVersions() {
    try {
      const res = await fetch(`/api/product-strategy-agent/versions?conversationId=${conversationId}`);
      if (res.ok) {
  ...
  async function handleCreateSnapshot() {
    ...
    try {
      const res = await fetch('/api/product-strategy-agent/versions', {
        method: 'POST',
    ...
    <div className="space-y-4">
      ...
      {versions.length === 0 ? (
        ...
        <>
          ...
          {left && right && <DiffPanel left={left} right={right} />}
        </>
      )}
    </div>
  );
