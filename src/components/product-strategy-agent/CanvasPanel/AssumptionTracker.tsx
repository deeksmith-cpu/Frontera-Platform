...
export function AssumptionTracker({ conversationId }: AssumptionTrackerProps) {
  ...
  async function loadAssumptions() {
    try {
      const res = await fetch(`/api/product-strategy-agent/assumptions?conversationId=${conversationId}`);
      if (res.ok) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/product-strategy-agent/assumptions', {
        method: 'POST',
