...
type ReviewTab = 'triggers' | 'assumptions' | 'signals' | 'versions';
...
export function ReviewSection({ conversation }: ReviewSectionProps) {
  ...
  async function loadReviewContext() {
    try {
      const [betsRes, assumptionsRes] = await Promise.all([
        fetch(`/api/product-strategy-agent/bets?conversation_id=${conversation.id}`),
        fetch(`/api/product-strategy-agent/assumptions?conversationId=${conversation.id}`),
      ]);
  ...
  return (
    <div className="space-y-6">
      {/* Section Header */}
      <div>
        <h2 className="text-lg font-bold text-slate-900">Living Strategy</h2>
        <p className="text-sm text-slate-500 mt-1">
          Track assumptions, log signals, and monitor your strategy&apos;s evolution over time.
        </p>
      </div>
                ...
                {triggers.map((trigger, idx) => {
                  ...
                  return (
                    <div key={idx} className={`${colors.bg} border ${colors.border} rounded-2xl p-4`}>
                      ...
                      <div suppressHydrationWarning className="mt-2 text-[10px] text-slate-400">
