...
export function BetsView({ conversation, onPhaseTransition }: BetsViewProps) {
  ...
  const handleContinueToActivation = useCallback(async () => {
    ...
    try {
      // Check quality gate first to get a user-visible error
      const checkRes = await fetch(
        `/api/product-strategy-agent/phase/quality-gate?conversation_id=${conversation.id}&phase=bets`
      );
      if (checkRes.ok) {
        const gate = await checkRes.json();
        if (!gate.passed) {
          setGateError(gate.message || 'Quality gate not met. Ensure you have 3+ bets with kill criteria and at least 1 thesis.');
          return;
        }
      }

      await onPhaseTransition('activation');
    } catch {
      setGateError('Something went wrong. Please try again.');
    } finally {
      setIsTransitioning(false);
    }
  }, [conversation.id, onPhaseTransition]);
        ...
        <div className="max-w-6xl mx-auto px-6 pb-8">
          <div className="bg-gradient-to-r from-emerald-50 to-cyan-50 border border-emerald-200 rounded-2xl p-5 text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <h3 className="text-sm font-bold text-slate-900">Ready for Activation?</h3>
            </div>
            <p className="text-xs text-slate-600 mb-4 max-w-md mx-auto leading-relaxed">
              Once your strategic bets are defined with clear hypotheses and kill criteria,
              move forward to build your activation plan.
            </p>
            ...
            {gateError && (
              <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 max-w-lg mx-auto text-left">
                ...
                <p className="text-xs text-amber-800 leading-relaxed">{gateError}</p>
              </div>
            )}
            ...
            >
              {isTransitioning ? 'Checking...' : 'Continue to Activation'}
